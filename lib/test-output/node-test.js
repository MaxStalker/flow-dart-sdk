...
const preparePayload = tx => {
  ...
  return [
    toArray(scriptBuffer(tx.cadence)),
    tx.arguments.map(argumentToString),
    toArray(blockBuffer(tx.refBlock)),
    tx.computeLimit,
    toArray(addressBuffer(tx.proposalKey.address)),
    tx.proposalKey.keyId,
    tx.proposalKey.sequenceNum,
    toArray(addressBuffer(tx.payer)),
    tx.authorizers.map((v) => toArray(addressBuffer(v))),
  ]
}
